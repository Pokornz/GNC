...
class quadrotor:
    def __init__(self, tag, dt, m, l, J, CDl, CDr, kt, km, kw, att,
            pqr, xyz, v_ned, w, B, De, Dn):
        # physical constants
        self.tag = tag
        self.dt = dt
        self.m = m   # [Kg]
        ...
        # States q = [[p_n],[p_e],[b]]
        self.c_q = np.array([[xyz[0]], [xyz[1]], [0]])
        self.c_P = np.array([[1, 0, 0],
                             [0, 1, 0],
                             [0, 0, 1]])
        self.c_F = np.array([[1, 0, dt],
                             [0, 1, dt],
                             [0, 0, 1]])

        # input u = [[u_vn], [u_ve]] (desired velocities)
        self.c_u = np.array([[0],[0]])
        self.c_G = np.array([[dt, 0],
                             [0, dt],
                             [0, 0]])
        # measuring only p_n and p_e
        self.c_H = np.array([[1, 0, 0],
                             [0, 1, 0]])
        # standard deviations of the input and the measurement
        # Should be zero?!
        self.c_Q = np.array([[0.0001, 0],
                             [0, 0.0001]])
        self.c_Pym = np.array([[0.0001, 0],
                               [0, 0.0001]])
    ...
    def set_v_2D_alt_lya(self, vxy_d, alt_d):
        self.c_u = np.array([[vxy_d[0]],[vxy_d[1]]])
        vxy_d = np.array([vxy_d[0]+self.c_bias, vxy_d[1]+self.c_bias])
    ...
    def set_auto_formation(self, p_n, p_e, alt_d):
        i = self.tag-1
        
        u_n = self.c_K * -self.B.dot(self.B.transpose().dot(p_n)-self.D_n)
        
        u_e = self.c_K * -self.B.dot(self.B.transpose().dot(p_e)-self.D_e)
        
        v_2D_d = np.array([u_n[i][0], u_e[i][0]])
        self.dB = la.norm(self.c_LEB-self.c_q[2][0])/self.dt
        if self.c_it*self.dt < 10:
            v_2D_d = np.array([0, 0])
        else:
            if not self.settled:
                v_2D_d = np.array([0, 0])
            if la.norm(self.c_LEB-self.c_q[2][0])/self.dt < 0.0001:
                self.settled = True
            
            
        bias = np.array([self.c_q[2][0], self.c_q[2][0]])
        self.set_v_2D_alt_lya(v_2D_d - bias, alt_d)
    ...
    def step(self, dt):
        ...
        self.c_it += 1
        # Estimation
        self.c_q = self.c_F.dot(self.c_q) + self.c_G.dot(self.c_u)
        self.c_P = self.c_F.dot(self.c_P).dot(self.c_F.transpose()) + self.c_G.dot(self.c_Q).dot(self.c_G.transpose())
        if self.c_it%10 == 0:
            # Correction
            self.c_LEB = self.c_q[2][0]
            q_saved = self.c_q
            P_saved = self.c_P
            Py = self.c_H.dot(P_saved).dot(self.c_H.transpose())
            K = P_saved.dot(self.c_H.transpose()).dot(la.inv(Py+self.c_Pym))
            ym = np.array([[self.xyz[0]],[self.xyz[1]]]) # measurement
            self.c_q = self.c_q + K.dot(ym - self.c_H.dot(self.c_q))
            self.c_P = self.c_P - K.dot(self.c_H).dot(self.c_P)
            if not np.all(la.eigvals(self.c_P) > 0):
                print("Non-positive P")
                self.c_q = q_saved
                self.c_P = P_saved
            # print(self.c_q[2])
            # print(ym)
